...
namespace xtUML1
{
    ...
    class Translate
    {
        ...
        {
            string translatedTypeScriptCode = string.Empty;
            try
            {
                // Read the JSON file
                ...
                translatedTypeScriptCode = sourceCodeBuilder.ToString();
            }
            catch (Exception ex)
            {
            ...
            return translatedTypeScriptCode;
        }
        ...
        private void GenerateAttribute(JsonData.Attribute1 attribute, JsonData json)
        {
            ...
            else if (attribute.data_type == "inst_event")
            {
                ...
                sourceCodeBuilder.AppendLine($"\n    public  {attribute.event_name}({cName} .{cName}) {{");
        ...
        private void GenerateConstructor(List<JsonData.Attribute1> attributes)
        {
            ...
            foreach (var attribute in attributes)
            {
                ...
                {
                    string dataType = MapDataType(attribute.data_type);
                    sourceCodeBuilder.Append($"TIMER {attribute.attribute_name}  : {dataType},");
                }
                else if (attribute.data_type == "inst_ref")
                {
        ...
        private void GenerateGetter(JsonData.Attribute1 getter)
        {
            ...
            {
                string dataType = MapDataType(getter.data_type);
                sourceCodeBuilder.AppendLine($"      public  get{getter.attribute_name}Ref() : {dataType}{{");
            ...
            {
                string dataType = MapDataType(getter.data_type);
                sourceCodeBuilder.AppendLine($"      public  get{getter.attribute_name}RefSet() : {dataType} {{");
        ...
        private void GenerateSetter(JsonData.Attribute1 setter)
        {
            ...
            {
                string dataType = MapDataType(setter.data_type);
                sourceCodeBuilder.AppendLine($"      public  set{setter.attribute_name}(TIMER {setter.attribute_name}) {{");
                sourceCodeBuilder.AppendLine($"        this.{setter.attribute_name} = {setter.attribute_name};");
                sourceCodeBuilder.AppendLine($"}}");
            }
            ...
            {
                string dataType = MapDataType(setter.data_type);
                sourceCodeBuilder.AppendLine($"      public  set{setter.attribute_name}Ref({setter.related_class_name} ${setter.attribute_name}Ref) {{");
            ...
            {
                string dataType = MapDataType(setter.data_type);
                sourceCodeBuilder.AppendLine($"      public  set{setter.attribute_name}RefSet({setter.related_class_name} ${setter.attribute_name}) {{");
        ...
        private void GenerateGetState(JsonData.Attribute1 getstate)
        {
            ...
            {
                string dataType = MapDataType(getstate.data_type);
                sourceCodeBuilder.AppendLine($"     public  GetState()  : {dataType} {{");
            ...
            foreach (var state in states)
            {
                if (state.state_event != null)
                {
                    for (int i = 0; i < state.state_event.Length; i++)
                    {
                        ...
                        sourceCodeBuilder.AppendLine($"    public  {methodName}() : void{{");
        ...
        private void Transition(List<JsonData.State> state, string status)
        {
            ...
            sourceCodeBuilder.AppendLine($"     public  Transition() {{");
